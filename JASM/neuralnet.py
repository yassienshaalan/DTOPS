...
class NeuralNet(object):
  ...
  def LoadModelOnGPU(self, batchsize=-1):
    ...
    input_datalayernames = [node.name for node in self.layer if node.is_input]
    self.output_datalayer = [node for node in self.layer if node.is_output]
    output_datalayer_names = [node.name for node in self.layer if node.is_output]
  ...
  def Sort(self):
    ...
    while S:
      ...
      for m in n.outgoing_edge:
        if m.marker == 0:
          m.marker = 1
          if functools.reduce(lambda a, edge: a and edge.marker == 1,
  ...
  def EvaluateOneBatch(self):
    ...
    #print("Inside evaluate one ")
    losses = self.ForwardPropagate()
    #print("losses forward ",losses)
    #print("evaluate one self.output_datalayer ",self.output_datalayer)
    losses.extend([node.GetLoss() for node in self.output_datalayer])
    #print("In evaluating losses are ",losses)
    return losses
  ...
  def Evaluate(self, validation=True, collect_predictions=False):
    ...
    while not stop:
      datagetter()
      #print("Will evaluate one batch")
      losses = self.EvaluateOneBatch()
      #print("losses after ",losses)
      if collect_predictions:
        #print("Collecting data from ",collect_pos,collect_pos + batchsize)
    ...
    if collect_predictions and stats:
      ...
      for i in range(len(predictions)):
          ...
          #print("Argmax is ",pred,predictions[i],targets[i],np.argmax(targets[i]))
          if pred == 0:
              new_pred.append(0)
          else:
              new_pred.append(1)
          tar = np.argmax(targets[i])
          if tar== 0:
              new_target.append(0)
          else:
              new_target.append(1)
      #new_pred=predictions
      print("lenghts",len(new_pred),len(new_target))
      print(classification_report(new_target, new_pred, target_names=target_names))
  ...
  def ScoreOneLabel(self, preds, targets):
    ...
    step = (-preds.T).argsort().flatten()
    #print("step is",step)   
    #targets_sorted = targets[step,:]
    targets_sorted = targets[step]
    #print("targets_sorted",len(targets_sorted),targets_sorted)
    #print("targets",len(targets),targets)
    ...
    #print("precsion is ",len(prec),prec)
    #print("recall is ",len(recall),recall)
  ...
  def ComputeScore(self, preds, targets):
    ...
    ap_list = []
    prec_list = []
    #print("numdims is is is is ",numdims)
  ...
  def SetUpData(self, skip_outputs=False, skip_layernames=[]):
    ...
    handles = GetDataHandles(op, name_list, hyp_list,verbose=self.verbose)
  ...
  def Train(self):
    ...
    try:
      ...
      print("Exception in getting performance measure")
      pass
    ...
    print("select_model_using_error",select_model_using_error,select_model_using_acc,select_model_using_map,select_best)
    if select_best:
    ...
    while not stop:
      ...
      if self.EvalNow(step):
        ...
        for stat in stats:
          ...
          sts = GetPerformanceStats(stat, prefix='T')
