...
reviews_train = reviews_train[:350000]
...
for revSent in tqdm(review_sent_all, total=len(review_sent_all)):
    ...
    if polarity_orig_all[index] == 0.0:
        indo += 1
...
text_file_path_test = "./data_text_embedding/spam_zip_run/rev_text_features_new_test.txt"
sentiment_file_path_test = "./data_text_embedding/spam_zip_run/rev_sentiment_features_new_test.txt"
text_file_path_train = "./data_text_embedding/spam_zip_run/rev_text_features_new_train.txt"
sentiment_file_path_train = "./data_text_embedding/spam_zip_run/rev_sentiment_features_new_train.txt"
...
for line in tqdm(out, total=len(out)):
    ...
    for j in range(len(all_sentiment_vector[index])):
        polarity_old = all_sentiment_vector[index][j]
        polarity = polarity_old  # *line[j]
        polarity_map = Remap(polarity, -1, 1, 0.1, 1)
