...
#reviews_train = reviews_train[:90000]
...
term_test = read_data_from_text(data_file_path_test)
term_train = read_data_from_text(data_file_path_train)

print("Reading Sentiment ")
data_file_path_test = "./Aspect_Datasets/"+folder+"/polarity_test.txt"
data_file_path_train = "./Aspect_Datasets/"+folder+"/polarity_train.txt"

polarity_test = read_data_from_text(data_file_path_test)
polarity_train = read_data_from_text(data_file_path_train)

print("Number of processed test reviews " + str(num_revs_test) + " for testing")

print("Number sent test ",len(reviews_test)," term_test ",len(term_test), " polarity_test ",len(polarity_test))
print("Number sent train ",len(reviews_train)," term_train ",len(term_train), " polarity_test ",len(polarity_train))


for i in range(len(reviews_test)):
    sent = reviews_test[i].split("\n")[0]
    if i == 1 or i == len(reviews_test)-1:
        print(sent)
    extra = ""
    for j in range(10):
        extra+=term_test[i]+" "
    ...
    if i == 1 or i == len(reviews_test)-1:
        print(sent+"\t"+extra)
    reviews_test[i]=sent+"\t"+extra

for i in range(len(reviews_train)):
    sent = reviews_train[i].split("\n")[0]
    if i == 1 or i == len(reviews_train)-1:
        print(sent)
    extra = ""
    for j in range(10):
        extra+=term_train[i]+" "
    if i == 1 or i == len(reviews_train)-1:
        print(sent+"\t"+extra)
    reviews_train[i]=sent+"\t"+extra

polarity_orig_overall = []
...
for i in range(len(all_reviews)):
    ...
    polarity_orig_overall.append(all_val)
    #Textblob way
    #polarity_orig_all.append(TextBlob(all_reviews[i]).sentiment.polarity)

print("polarity_orig_overall", len(polarity_orig_overall))  # ,polarity_orig_all)
...
for revSent in tqdm(review_sent_all, total=len(review_sent_all)):
    ...
    if extract_sentiment ==1:
        ...
        if polarity_orig_overall[index][0] == 0.0 and polarity_orig_overall[index][1] == 0.0 and polarpolarity_orig_overallity_orig_all[index][2] == 0.0 and polarity_orig_overall[index][3] == 0.0:
            indo += 1
        sentiment_vector.append(polarity_orig_overall[index])
        if indo == len(vocab) + 1:
            nzero += 1
            '''
            fold = "train"
...
folder = "sem_eval_15_supervised_run"
...
for line in tqdm(out, total=len(out)):
    ...
    if index >= num_revs_train:
        ...
        if extract_sentiment ==1:
            if polarity_test[index - num_revs_train]=="positive":
                sent_feat+=str(50)+"\t"+str(50)+"\t"+str(50)+"\t"+str(50)+"\t"
            elif polarity_test[index - num_revs_train]=="negative":
                sent_feat+=str(-50)+"\t"+str(-50)+"\t"+str(-50)+"\t"+str(-50)+"\t"
            else:
                sent_feat+=str(10)+"\t"+str(10)+"\t"+str(10)+"\t"+str(10)+"\t"
        ...
        if extract_sentiment == 1:
            if polarity_train[index]=="positive":
                sent_feat+=str(50)+"\t"+str(50)+"\t"+str(50)+"\t"+str(50)+"\t"
            elif polarity_train[index]=="negative":
                sent_feat+=str(-50)+"\t"+str(-50)+"\t"+str(-50)+"\t"+str(-50)+"\t"
            else:
                sent_feat+=str(10)+"\t"+str(10)+"\t"+str(10)+"\t"+str(10)+"\t"
