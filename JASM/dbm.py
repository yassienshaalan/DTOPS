...
class DBM(NeuralNet):
  ...
  def __init__(self, *args, **kwargs):
    ...
    print("Here i am creating a DBM")
  ...
  def DumpModelState(self, step):
    ...
    filename = '/research/remote/petabyte/users/yassien/Joint_Aspect_Sentiment_Model/Asp_Sent_joint_spam_ny/states/%s_%d' % (self.net.name, step)
  ...
  """
    if layer.is_input and not compute_input:
      ...
      #print("layer is ",layer,layer.name,layer.params)
      b = layer.params['bias']
      #print("replicated_neighbour "+str(layer)+" "+str(layer.replicated_neighbour)) 
  ...
  def WriteRepresentationToDisk(self, layernames, output_dir, memory='2G',
                                dataset='test', input_recon=False):
    print("in DBM writing module")
    print("layernames",layernames)
    layers = [self.GetLayerByName(lname) for lname in layernames]
    print("layers ",layers,"my name is ")
    ...
    if dataset == 'train':
      ...
      if self.train_data_handler is None:
        return
      numbatches = self.train_data_handler.num_batches#math.ceil(self.train_data_handler.num_batches)
      size = numbatches * self.train_data_handler.batchsize#math.ceil(numbatches * self.train_data_handler.batchsize)
    elif dataset == 'validation':
      datagetter = self.GetValidationBatch
      if self.validation_data_handler is None:
        print("Validation data handler error")
        return
      numbatches = self.validation_data_handler.num_batches#math.ceil(self.validation_data_handler.num_batches)
      size = numbatches * self.validation_data_handler.batchsize#math.ceil(numbatches * self.validation_data_handler.batchsize)
    elif dataset == 'test':
      datagetter = self.GetTestBatch
      if self.test_data_handler is None:
        return
      numbatches = self.test_data_handler.num_batches#math.ceil(self.test_data_handler.num_batches)
      
      size = numbatches * self.test_data_handler.batchsize#math.ceil(numbatches * self.test_data_handler.batchsize)
      
      
    print("numbatches dbm from DBM ",numbatches)
    print("size from DBM ",size)
    print("layernames DBM ",layernames)
    print("numdim_list DBM",numdim_list)
    datawriter = DataWriter(layernames, output_dir, memory, numdim_list, size)
    #numbatches = int(numbatches)
    #write_to_dataset = 'train' or write_to_dataset = 'validation':
    
    if dataset ==  'train' or dataset == 'validation':#write_to_dataset:
        new_file_path = output_dir+'/'+dataset+'_'+layernames[0]+'.txt'
    ...
    for batch in range(numbatches):
      ...
      if dataset ==  'train' or dataset == 'validation':#== write_to_dataset:
          ...
          for i in range(len(reprs)):
              for j in range(len(reprs[i])):
                  result = ""
                  for k in range(len(reprs[i][j])):
                      result+=str(reprs[i][j][k])+'\t'
                  allrecs.append(reprs[i][j]) 
                  filehandle.write(result+"\n")
                  count+=1
      
      #print(len(reprs),type(reprs),len(reprs[0]),len(reprs[0][1]),reprs[0][1])
      datawriter.Submit(reprs)
    if dataset ==  'train' or dataset == 'validation':
        filehandle.close()
        print(" wrote my way ",count," records to ",new_file_path)
    sys.stdout.write('\n')
