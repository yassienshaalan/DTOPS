...
class Disk(object):
  ...
  def Get(self, batchsize):
    ...
    print(" batchsize from datahandler ",str(batchsize))
    print(" self.num_data in datahandler ",self.num_data)
    for i in range(self.num_data):
      key = self.keys[i]
      
      numdims = int(self.numdim_list[i])
...
def GetDataHandles(op, names, hyp_list, verbose=False):
  ...
  #print("data_proto_file",data_proto_file)
  dataset_proto = util.ReadData(data_proto_file)
  #print("here data_proto_file ",data_proto_file)
  handlers = []
  if dataset_proto.data_handler == 'deepnet':
    ...
    for name_list in names:
      ...
      for name in name_list:
        ...
        try:
          ...
          print ('%s not found in data pbtxt' % name,dataset_proto.data)
          raise e
        datasetsize = data_proto.size
        numdims = np.prod(np.array(data_proto.dimensions))
        size += datasetsize * numdims * typesize
        print("name",name,"datasetsize",datasetsize ,"numdims",numdims,"typesize",typesize,"size",size)  
      size_list.append(size)
    total_size = sum(size_list)
    #print("size_list",size_list,"total_size",total_size)
    if total_size>0:
    ...
    for i, name_list in enumerate(names):
      if name_list == []:
        ...
        print("Creating handle here ",name_list)
...
class DataHandler(object):
  ...
  def __init__(self, op, data_name_list, hyperparameter_list, frac=1.0):
    ...
    for name, hyp in zip(data_name_list, hyperparameter_list):
      data_proto = next(d for d in dataset_proto.data if d.name == name)
      #print("data proto aho",name)
      #print(data_proto)
      file_pattern = os.path.join(dataset_proto.prefix, data_proto.file_pattern)
      datasetsize = data_proto.size
      #print("file_pattern",file_pattern)
      sorted_out = sorted(glob.glob(file_pattern))
      #if len(sorted_out)==0 and '.'in file_pattern:
      #   sorted_out.append(file_pattern) 
      #print("Sorted out ",sorted_out)
      filenames.append(sorted_out)
      #print("filenames now ",filenames)
      if data_proto.stats_file!='':
      ...
      if datasetsize is None:
        #print("datasize is none for ",name," setting to ",data_proto.size)
        datasetsize = data_proto.size
      else:
        #print("datasetsize is ",datasetsize," while data_proto.size ",data_proto.size,data_proto.name)
        assert datasetsize == data_proto.size, 'Size of %s is not %d' % (name, datasetsize)
    ...
    if self.verbose:
      if seq:
        ...
        print("Batches in disk: " + str(leftovers) + ' '+str((datasetsize / batchsize))+ ' '+str(datasetsize % batchsize))
    ...
    self.num_batches = int(datasetsize / batchsize)#math.ceil(datasetsize / batchsize)
    print("self.num_batches is ",self.num_batches)
    if self.get_last_piece and datasetsize % batchsize > 0:
      print("Adding an extra batch to collect the rest")
      self.num_batches += 1
