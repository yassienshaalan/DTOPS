...
def _obtain_lock(id):
    ...
    file = _lock_file(id)
    #print(file)
    #print(os.symlink('/dev/null',file))
    try:
...
def _launch_reaper(id, pid):
    ...
    #print("me "+str(me))
    #print("myloc "+str(myloc))
    if not myloc:
        myloc = os.getcwd()
    #print("myloc "+str(myloc))
    reaper_cmd = os.path.join(myloc, 'run_on_me_or_pid_quit')
    #print(reaper_cmd)
    #print("Now excecuting the command")
    #print("input is "+str(pid), me, '--free', str(id)	)
    #print(Popen([reaper_cmd, str(pid), me, '--free', str(id)],stdout=open('/dev/null', 'w')))
